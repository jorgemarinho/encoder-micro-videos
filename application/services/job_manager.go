...
type JobManager struct {
	...
	JobReturnChanel chan JobWorkerResult
...
func NewJobManager(db *gorm.DB, rabbitMQ *queue.RabbitMQ, jobReturnChanel chan JobWorkerResult, messageChannel chan amqp.Delivery) *JobManager {
	...
	return &JobManager{
		...
		JobReturnChanel: jobReturnChanel,
		RabbitMQ:        rabbitMQ,
	}
}
...
func (j *JobManager) Start(ch *amqp.Channel) {
	...
	if err != nil {
		log.Fatalf("error loadind var: CONCURRENCY_WORKERS.")
	}

	for qtdProcesses := 0; qtdProcesses < concurrency; qtdProcesses++ {
		go JobWorker(j.MessageChanel, j.JobReturnChanel, jobService, j.Domain, qtdProcesses)
	}

	for jobResult := range j.JobReturnChanel {
...
func (j *JobManager) checkParseErrors(jobResult JobWorkerResult) error {
	if jobResult.Job.ID != "" {
		log.Printf("MessageID: %v. Error during the job: %v with the video: %v. Error: %v",
